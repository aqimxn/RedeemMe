...
import redeemMe.Order.dao.OrderDAO;
...
public class ViewDashboard extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.setAttribute("userCount", UserDAO.getUsersCount());
		request.setAttribute("orderCount", OrderDAO.getOrdersCount());
		request.setAttribute("productCount", ProductDAO.getProductsCount());
		request.setAttribute("supplierCount", SupplierDAO.getSuppliersCount());
		request.setAttribute("sessionId", request.getSession().getId());
		
		//Obtain the RequestDispatcher from the request object. The pathname to the resource is listUser.jsp
		RequestDispatcher req = request.getRequestDispatcher("admin-dashboard.jsp");
