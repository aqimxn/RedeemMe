...
<html lang="en">
  <head>
    <meta charset="utf-8">
      ...
      <link rel="stylesheet" href="assets/css/admin-product-view.css">
      <link rel="stylesheet" href="assets/css/admin-product-update.css">
  ...
  <body class="  ">
    ...
    <!-- Update Product -->
  	<%
        String successUpdateProduct = (String) request.getAttribute("successUpdateProduct");
        if (successUpdateProduct != null) {
    %>
    <script>
        alert('<%= successUpdateProduct %>');
    </script>
    <%
        // Remove the attribute to prevent it from appearing again on page refresh
        request.removeAttribute("successUpdateProduct");
    }
	%>
	
		<%
	        String failUpdateProduct = (String) request.getAttribute("failUpdateProduct");
	        if (failUpdateProduct != null) {
	    %>
	    <script>
	        alert('<%= failUpdateProduct %>');
	    </script>
	    <%
	        // Remove the attribute to prevent it from appearing again on page refresh
	        request.removeAttribute("failUpdateProduct");
	    }
		%>
	
	<!-- Delete Product -->
	<%
        String successDeleteProduct = (String) request.getAttribute("successDeleteProduct");
        if (successDeleteProduct != null) {
    %>
    <script>
        alert('<%= successDeleteProduct %>');
    </script>
    <%
        // Remove the attribute to prevent it from appearing again on page refresh
        request.removeAttribute("successDeleteProduct");
    }
	%>
	
		<%
	        String failDeleteProduct = (String) request.getAttribute("failDeleteProduct");
	        if (failDeleteProduct != null) {
	    %>
	    <script>
	        alert('<%= failDeleteProduct %>');
	    </script>
	    <%
	        // Remove the attribute to prevent it from appearing again on page refresh
	        request.removeAttribute("failDeleteProduct");
	    }
		%>
	
	<!-- Add Product -->
	<%
        String successAddProduct = (String) request.getAttribute("successAddProduct");
        if (successAddProduct != null) {
    %>
    <script>
        alert('<%= successAddProduct %>');
    </script>
    <%
        // Remove the attribute to prevent it from appearing again on page refresh
        request.removeAttribute("successAddProduct");
    }
	%>
	
		<%
	        String failAddProduct = (String) request.getAttribute("failAddProduct");
	        if (failAddProduct != null) {
	    %>
	    <script>
	        alert('<%= failAddProduct %>');
	    </script>
	    <%
	        // Remove the attribute to prevent it from appearing again on page refresh
	        request.removeAttribute("failAddProduct");
	    }
		%>
    </footer>
