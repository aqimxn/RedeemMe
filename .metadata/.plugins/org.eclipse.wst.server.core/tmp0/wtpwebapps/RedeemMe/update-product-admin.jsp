...
<html lang="en">
  <head>
    <meta charset="utf-8">
      ...
      <link rel="stylesheet" href="assets/css/admin-product-update.css"></head>
    ...
    <div class="wrapper">
      ...
      <div class="iq-sidebar  sidebar-default ">
       ...
       <div id="modal-update" class="modal">
        ...
        <form action="UpdateProductController?productId=${product.productId}" method="POST" enctype="multipart/form-data">
            <div class="form first">
                <span id="modal-close" class="close" onclick="closeUpdateProduct()">&times;</span>
                <div class="details personal">
                    <span class="title">Edit Product Details - <b>(( ID: <c:out value="${product.productId}"/> ))</b></span>

                    <div class="fields">
                        <div class="input-field">
                            <label>Name</label>
                            <input type="text" name="productName" class="form-control" placeholder="Enter Product Name" value="<c:out value="${product.productName}"/>" required readonly style="cursor: not-allowed;">
                        </div>

                        <div class="input-field">
                           <label>Category</label>
                           <input type="text" name="productCategory" class="form-control" placeholder="Enter Product Category" value="<c:out value="${product.productCategory}"/>" required readonly style="cursor: not-allowed;">               
                        </div>

                        <div class="input-field">
                            <label>Points Required</label>
                            <input type="number" min="1" name="productPoints" value="<c:out value="${product.productPoints}"/>" required>
                        </div>

						<c:choose>
						    <c:when test="${product.supplierId == null}">
						        <c:set var="supplierValue" value="-"/>
						    </c:when>
						    <c:otherwise>
						        <c:set var="supplierValue" value="${PRODUCT.supplierName}"/>
						    </c:otherwise>
						</c:choose>
						
						<div class="input-field">
						    <label>Supplier</label>
						    <input type="text" name="productSupplier" class="form-control"
						           value="${supplierValue}"
                        ...
                        <div class="input-field">
                            ...
                            <input type="text" name="productQuantity" class="form-control" value="<c:out value="${product.productQuantity}"/>" readonly style="cursor: not-allowed;">
                        </div>

                        <div class="input-field">
                            <label>Add Quantity</label>
                            <input type="number" name="productAddQuantity" placeholder="Add Quantity" min="0" value="0" required>
                        </div>
                        
                        <div class="input-field">
                            <label>Description</label>
                            <input type="text" name="productDescription" value="<c:out value="${product.productDescription}"/>" >
                        </div>

                        <div class="input-field">
                            <label>Product Picture</label>
                            <input type="file" id="image" name="productImage" accept="image/jpeg, image/png" required>
                        </div>
                    </div>
                </div>

                    <div class="submit-btn">
                        <button type="button" onclick="closeUpdateProduct()" style="background-color: #E08DB4;">
                            Cancel
