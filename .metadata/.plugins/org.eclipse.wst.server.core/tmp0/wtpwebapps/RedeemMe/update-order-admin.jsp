...
<html lang="en">
  <head>
    <meta charset="utf-8">
      ...
      <link rel="stylesheet" href="assets/css/admin-order-view.css">
      <link rel="stylesheet" href="assets/css/admin-order-update.css">  </head>
    </head>
    ...
    <div class="wrapper">
      ...
      <div class="iq-sidebar  sidebar-default ">
					        ...
					        <tr>
								...
								<td>
								    <div class="d-flex align-items-center list-action justify-content-center">
								        <a href="ViewOrderController?orderId=${Order.orderId}" class="badge badge-info mr-2" 
								           data-toggle="tooltip" data-placement="top" title="View">
								            <i class="ri-eye-line mr-0"></i>
								        </a>
								        
								        <c:choose>
								            <c:when test="${Order.shipmentStatus != 'Delivered'}">
								                <a href="ToUpdateOrder?orderId=${Order.orderId}" class="badge bg-success mr-2" 
								                   data-toggle="tooltip" data-placement="top" title="Edit" onclick="updateStatusOptions()">
       ...
       <!-- Modal Edit -->
           ...
           <!-- Modal Update -->
            <div id="modal-update" data-current-status="${order.shipmentStatus}">
                <div class="modal-container-update">
                    <span id="modal-close" class="close-button" onclick="closeUpdateOrder()">&times;</span>
                    <h2>Update Shipment Status</h2>
                        <form action="UpdateOrderController?orderId=${order.orderId}" id="statusForm" method="POST" onchange="updateStatusOptions()" onclick="updateStatusOptions()">
                            <label for="status">Shipment Status:</label>
				 			<select id="status" name="shipment-status" required onchange="updateStatusOptions()">
				                <option value="Pending" <c:if test="${order.shipmentStatus == 'Pending'}">selected</c:if>>Pending</option>
				                <option value="Processed" <c:if test="${order.shipmentStatus == 'Processed'}">selected</c:if>>Processed</option>
				                <option value="Shipped" <c:if test="${order.shipmentStatus == 'Shipped'}">selected</c:if>>Shipped</option>
				                <option value="Delivered" <c:if test="${order.shipmentStatus == 'Delivered'}">selected</c:if>>Delivered</option>
				            </select>
                            <div class="btn">
                                <button id="submit-btn" type="submit">Update</button>
                                <button type="button" id="back-button" onclick="closeUpdateOrder()" style="background-color: #E08DB4;">
                                    Back
